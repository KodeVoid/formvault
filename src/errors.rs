...
impl fmt::Display for FormVaultError {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match self {
            FormVaultError::DatabaseError(s) => write!(formatter, "There is an error in the database: {}", s),
            ...
            FormVaultError::ValidationFailed(errors) => write!(formatter, "Validation failed: {}", errors.join(", ")),
        }
    }
}